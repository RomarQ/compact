...
The following flags, if present, affect the compiler's behavior as follows:
  ...
  --analyzed-ir causes the compiler to also write compiler/analyzed-ir.sexp
    into the target directory: the analyzed program in the compiler's own
    intermediate-language vocabulary, with each ledger operation and emit
    expanded to its Impact VM instructions.  An SDK in any language can
    execute circuits from it.  The format follows the intermediate language,
    so it can change between compiler versions.
"))
